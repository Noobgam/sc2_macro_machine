...
float ThreatAnalyzer::getUnitTypeThreat(const UnitType &unit) {
#pragma region PROTOSS
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_IMMORTAL)) {
        return 3.f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_VOIDRAY)) {
        return 2.6f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_COLOSSUS)) {
        return 2.1f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_ADEPT)) {
        // technically 1 stalker easykills adept, but it takes a really long time to do so.
        return 1.5f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_ADEPTPHASESHIFT)) {
        // hard to estimate threat of an adept
        return 1.5f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_DARKTEMPLAR)) {
        return 1.5f;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_STALKER)) {
        return 1;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_PHOTONCANNON)) {
        return 2;
    }
    if (unit.is(sc2::UNIT_TYPEID::PROTOSS_SHIELDBATTERY)) {
        return 4;
    }
    if (unit.isWorker()) {
...
#pragma region TERRAN
    if (unit.is(sc2::UNIT_TYPEID::TERRAN_MARINE)) {
