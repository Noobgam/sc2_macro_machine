...
constexpr static int VISIBILITY_PROBES = 7;

bool MapTools::isVisible(const CCTilePosition &from, const CCTilePosition &to, UnitType type, float additionalR) const {
    ...
    int dx = from.x - to.x;
    int dy = from.y - to.y;
    float R = type.visionRange() + additionalR;
    if (dx * dx + dy * dy > R * R) {
        return false;
    }
    ...
    float endx =   to.x + .5;
    float endy =   to.y + .5;
    for (int i = 1; i < VISIBILITY_PROBES; ++i) {
